...
/**
 * The main GUI for the Universal Pokemon Randomizer FVX, containing the various
 * options available and such.
 */
public class RandomizerGUI {
    private JCheckBox ptNewCustomCheckbox;
    private JCheckBox ptCustomCheckbox;
    ...
    private JCheckBox ptChangeNamesToTypesCheckBox;
    ...
    private JButton gen6EditorButton;
    private JButton gen7EditorButton;
    ...
    private boolean typeEffectivenessManuallyEdited;
    ...
    public RandomizerGUI() {
        ptNewCustomCheckbox = new JCheckBox("New Custom Checkbox");
        ptCustomCheckbox = new JCheckBox("Enable Custom Type");
        ...
        limitPokemonButton.addActionListener(e -> {
            ...
            if (gld.pressedOK()) {
                ...
                boolean isTypeTheme = isTrainerSetting(TRAINER_TYPE_THEMED)
    ...
    private void showInitialPopup() {
        ...
        if (initialPopup) {
            ...
            JLabel label = new JLabel("<html><a href=\"" + SysConstants.WIKI_IMPORTANT_INFO_URL
                    + "\">Checking out the \"Important Information\" page on the Wiki is highly recommended.</a>");
     ...
     * <li>Tell the GUI to react to a ROM having been opened, or create an error
     * dialog if the opening failed.</li>
     * </ol>
     * Returns the Thread.
     * 
     * @param f            The {@link File} to be opened as a ROM.
     * @param reinitialize If true, the load dialog will not be shown, and the GUI
     *                     will not react to a ROM being opened.
     */
    private Thread openRom(File f, boolean reinitialize) {
        // A rather simple method - make the romOpener open the file and react to its
        // results -
        // complicated by the need of an animated loading dialog and thus
        // multithreading...
    ...
    // This being public is not very pretty, but it works to get this code to
    // PresetLoadDialog without copy-pasting
    ...
    private void presetLoader() {
        ...
        if (pld.isCompleted()) {
            ...
            if (allowed && fh != null) {
                ...
                presetMode = true;
                performRandomization(fh.getAbsolutePath(), seed, pld.getCustomNames(), customPlayerGraphics,
    ...
    private SaveType askForSaveType() {
        ...
        if (romHandler.hasGameUpdateLoaded()) {
            ...
            JLabel label = new JLabel("<html><a href=\"" + SysConstants.WIKI_3DS_INFO_URL
                    + "#changes-to-saving-a-rom-when-working-with-3ds-games\">For more information, click here.</a>");
            label.addMouseListener(new MouseAdapter() {
                @Override
                public void mouseClicked(MouseEvent e) {
                    Desktop desktop = java.awt.Desktop.getDesktop();
                    try {
                        desktop.browse(new URI(SysConstants.WIKI_3DS_INFO_URL
    ...
    private void restoreStateFromSettings(Settings settings) {
        ...
        ptChangeNamesToTypesCheckBox.setSelected(settings.isChangeNamesToTypes());
        ...
        ptRandomFollowEvolutionsRadioButton
                .setSelected(settings.getSpeciesTypesMod() == Settings.SpeciesTypesMod.RANDOM_FOLLOW_EVOLUTIONS);
        ptRandomCompletelyRadioButton
                .setSelected(settings.getSpeciesTypesMod() == Settings.SpeciesTypesMod.COMPLETELY_RANDOM);
        ...
        tpUnchangedBattleStyleRadioButton
                .setSelected(settings.getBattleStyle().getModification() == BattleStyle.Modification.UNCHANGED);
        tpRandomBattleStyleRadioButton
                .setSelected(settings.getBattleStyle().getModification() == BattleStyle.Modification.RANDOM);
        tpSingleStyleRadioButton
                .setSelected(settings.getBattleStyle().getModification() == BattleStyle.Modification.SINGLE_STYLE);
        tpBattleStyleCombobox
                .setSelectedItem(selectableBattleStyles.get(settings.getBattleStyle().getStyle().ordinal()));
        ...
        wpZoneEncounterSetRadioButton
                .setSelected(settings.getWildPokemonZoneMod() == Settings.WildPokemonZoneMod.ENCOUNTER_SET);
        wpZoneMapRadioButton.setSelected(settings.getWildPokemonZoneMod() == Settings.WildPokemonZoneMod.MAP);
        wpZoneNamedLocationRadioButton
                .setSelected(settings.getWildPokemonZoneMod() == Settings.WildPokemonZoneMod.NAMED_LOCATION);
        ...
        wpTRThemedAreasRadioButton
                .setSelected(settings.getWildPokemonTypeMod() == Settings.WildPokemonTypeMod.RANDOM_THEMES);
        ...
        wpERBasicOnlyRadioButton
                .setSelected(settings.getWildPokemonEvolutionMod() == Settings.WildPokemonEvolutionMod.BASIC_ONLY);
        wpERSameEvolutionStageRadioButton
                .setSelected(settings.getWildPokemonEvolutionMod() == Settings.WildPokemonEvolutionMod.KEEP_STAGE);
        ...
        teUnchangedRadioButton
                .setSelected(settings.getTypeEffectivenessMod() == Settings.TypeEffectivenessMod.UNCHANGED);
        teRandomRadioButton.setSelected(settings.getTypeEffectivenessMod() == Settings.TypeEffectivenessMod.RANDOM);
        teRandomBalancedRadioButton
                .setSelected(settings.getTypeEffectivenessMod() == Settings.TypeEffectivenessMod.RANDOM_BALANCED);
        teKeepTypeIdentitiesRadioButton
                .setSelected(settings.getTypeEffectivenessMod() == Settings.TypeEffectivenessMod.KEEP_IDENTITIES);
    ...
    private Settings createSettingsFromState(CustomNamesSet customNames) {
        ...
        settings.setSpeciesTypesMod(ptUnchangedRadioButton.isSelected(),
                ptRandomFollowEvolutionsRadioButton.isSelected(),
        ...
        settings.setChangeNamesToTypes(ptChangeNamesToTypesCheckBox.isSelected());
        ...
        settings.setStartersTypeMod(spTypeNoneRadioButton.isSelected(), spTypeFwgRadioButton.isSelected(),
                spTypeTriangleRadioButton.isSelected(),
        ...
        settings.setBattleStyleMod(tpUnchangedBattleStyleRadioButton.isSelected(),
                tpRandomBattleStyleRadioButton.isSelected(), tpSingleStyleRadioButton.isSelected());
        settings.setSingleStyleSelection(isBattleStyle(SINGLE_BATTLE), isBattleStyle(DOUBLE_BATTLE),
                isBattleStyle(TRIPLE_BATTLE), isBattleStyle(ROTATION_BATTLE));
        ...
        settings.setTypeEffectivenessMod(teUnchangedRadioButton.isSelected(), teRandomRadioButton.isSelected(),
                teRandomBalancedRadioButton.isSelected(), teKeepTypeIdentitiesRadioButton.isSelected(),
                teInverseRadioButton.isSelected());
    ...
    private void initialState() {
        ...
        setInitialButtonState(ptUnchangedRadioButton, ptRandomFollowEvolutionsRadioButton,
                ptRandomCompletelyRadioButton,
        ...
        setInitialButtonState(pmsUnchangedRadioButton, pmsRandomPreferringSameTypeRadioButton,
        ...
        // TODO: why do these checkboxes exist? can't they just be generated from the
        // MiscTweak objects?
        // Well, this lets them be named variables, which helps for code readability if
        // nothing else...
     ...
     * If any disabled button is selected, changes that selection to the default
     * button given.
     ...
     * however, consider using disableAndDeselectButtons and/or
     * disableButtonsWithDefault instead.
     * 
     * @param buttons The buttons to disable.
     */
    ...
    private void romLoaded() {
        ...
        try {
            ...
            boolean heldItemsAvailable = canAddHeldItemsToBoss || canAddHeldItemsToImportant
                    || canAddHeldItemsToRegular;
            ...
            // We shouldn't use setVisible on the panels directly in the tabbedPane; it
            // causes strange bleedover
            // Disable it instead
            ...
            typeEffectivenessManuallyEdited = false;
            refreshTypeEditorButtonLabel();
            updateTypeEditorButtonState(typeSupport);
            ...
            gen6EditorButton.setEnabled(pokemonGeneration == 6);
            gen7EditorButton.setEnabled(pokemonGeneration == 7);
    ...
    private void enableOrDisableSubControls() {
        ...
        if (ptUnchangedRadioButton.isSelected()) {
            disableAndDeselectButtons(ptFollowMegaEvosCheckBox, ptIsDualTypeCheckBox, ptChangeNamesToTypesCheckBox);
        } else {
            ptFollowMegaEvosCheckBox.setEnabled(followMegaEvolutionControlsEnabled);
            ptIsDualTypeCheckBox.setEnabled(true);
            ptChangeNamesToTypesCheckBox.setEnabled(true);
        }
    ...
    private void initTypeEffectivenessEditorUI() {
        if (teUnchangedRadioButton == null || bundle == null) {
            return;
        }
        if (teManualEditButton != null) {
            return;
        }

        JPanel typeEffectivenessPanel = (JPanel) SwingUtilities.getAncestorOfClass(JPanel.class,
                teUnchangedRadioButton);
        if (typeEffectivenessPanel == null) {
            return;
        }

        teManualEditButton = new JButton(bundle.getString("GUI.teManualEditorButton.labelDefault"));
        teManualEditButton.setName("teManualEditButton");
        teManualEditButton.addActionListener(e -> openTypeEffectivenessEditor());
        ...
        typeEffectivenessPanel.revalidate();
        typeEffectivenessPanel.repaint();

        updateTypeEditorButtonState(false);
        refreshTypeEditorButtonLabel();
    }

    private void updateTypeEditorButtonState(boolean typeSupport) {
        if (teManualEditButton == null || bundle == null) {
            return;
        }
        teManualEditButton.setEnabled(typeSupport);
        String tooltipKey = typeSupport ? "GUI.teManualEditorButton.toolTipText"
                : "GUI.teManualEditorButton.toolTipTextDisabled";
        teManualEditButton.setToolTipText(bundle.getString(tooltipKey));
        teManualEditButton.setVisible(true);
    }

    private void refreshTypeEditorButtonLabel() {
        if (teManualEditButton == null || bundle == null) {
            return;
        }
        String labelKey = typeEffectivenessManuallyEdited ? "GUI.teManualEditorButton.labelCustom"
                : "GUI.teManualEditorButton.labelDefault";
        teManualEditButton.setText(bundle.getString(labelKey));
    }

    private void openTypeEffectivenessEditor() {
        if (romHandler == null || !romHandler.hasTypeEffectivenessSupport()) {
            JOptionPane.showMessageDialog(frame, bundle.getString("GUI.teManualEditorDialog.unsupported"),
                    bundle.getString("GUI.teManualEditorDialog.errorTitle"), JOptionPane.WARNING_MESSAGE);
            return;
        }

        TypeTable currentTable = romHandler.getTypeTable();
        if (currentTable == null) {
            JOptionPane.showMessageDialog(frame, bundle.getString("GUI.teManualEditorDialog.loadError"),
                    bundle.getString("GUI.teManualEditorDialog.errorTitle"), JOptionPane.ERROR_MESSAGE);
            return;
        }

        if (!teUnchangedRadioButton.isSelected()) {
            int result = JOptionPane.showConfirmDialog(frame,
                    bundle.getString("GUI.teManualEditorDialog.randomWarning"),
                    bundle.getString("GUI.teManualEditorDialog.title"), JOptionPane.OK_CANCEL_OPTION,
                    JOptionPane.WARNING_MESSAGE);
        ...
        if (dialog.wasSaved()) {
            ...
            refreshTypeEditorButtonLabel();
            enableOrDisableSubControls();
            JOptionPane.showMessageDialog(frame, bundle.getString("GUI.teManualEditorDialog.randomizationCleared"),
                    bundle.getString("GUI.teManualEditorDialog.infoTitle"), JOptionPane.INFORMATION_MESSAGE);
        }
    }
    ...
    private void initPokemonEditorsPanel() {
        // Create the panel with GridBagLayout
        pokemonEditorsPanel = new JPanel(new GridBagLayout());
        TitledBorder border = BorderFactory.createTitledBorder(bundle.getString("GUI.pokemonEditorsPanel.title"));
        border.setTitleFont(border.getTitleFont().deriveFont(Font.BOLD));
        pokemonEditorsPanel.setBorder(border);
        ...
        JLabel descriptionLabel = new JLabel(bundle.getString("GUI.pokemonEditorsPanel.description"));
        c.gridx = 0;
        c.gridy = 0;
        c.gridwidth = 2;
        ...
        gen3EditorButton.addActionListener(e -> openGen3Editor());

        gen4EditorButton = new JButton(bundle.getString("GUI.gen4EditorButton.text"));
        gen4EditorButton.setToolTipText(bundle.getString("GUI.gen4EditorButton.toolTipText"));
        gen4EditorButton.setEnabled(false);
        gen4EditorButton.addActionListener(e -> openGen4Editor());

        gen5EditorButton = new JButton(bundle.getString("GUI.gen5EditorButton.text"));
        gen5EditorButton.setToolTipText(bundle.getString("GUI.gen5EditorButton.toolTipText"));
        gen5EditorButton.setEnabled(false);
        gen5EditorButton.addActionListener(e -> openGen5Editor());

        gen6EditorButton = new JButton(bundle.getString("GUI.gen6EditorButton.text"));
        gen6EditorButton.setToolTipText(bundle.getString("GUI.gen6EditorButton.toolTipText"));
        gen6EditorButton.setEnabled(false);
        gen6EditorButton.addActionListener(e -> openGen6Editor());

        gen7EditorButton = new JButton(bundle.getString("GUI.gen7EditorButton.text"));
        gen7EditorButton.setToolTipText(bundle.getString("GUI.gen7EditorButton.toolTipText"));
        gen7EditorButton.setEnabled(false);
        gen7EditorButton.addActionListener(e -> openGen7Editor());

        // Add buttons in a grid layout (2 columns)
        c.gridwidth = 1;
        c.gridy = 1;
        ...
        // Add the Pokemon Editors tab to the tabbed pane
        JScrollPane scrollPane = new JScrollPane(pokemonEditorsPanel);
    ...
    private void openGen4Editor() {
        if (romHandler == null) {
            JOptionPane.showMessageDialog(frame,
                    "No ROM loaded. Please load a Gen 4 ROM first.",
                    "Error",
                    JOptionPane.ERROR_MESSAGE);
            return;
        }

        // Launch the Gen 4 Editor
        SwingUtilities.invokeLater(new Runnable() {
            @Override
            public void run() {
                com.dabomstew.pkrandom.pokemon.editors.Gen4EditorFrame editor = new com.dabomstew.pkrandom.pokemon.editors.Gen4EditorFrame(
                        romHandler);
    ...
    private void openGen5Editor() {
        if (romHandler == null) {
            JOptionPane.showMessageDialog(frame,
                    "No ROM loaded. Please load a Gen 5 ROM first.",
                    "Error",
                    JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        SwingUtilities.invokeLater(new Runnable() {
            @Override
            public void run() {
                com.dabomstew.pkrandom.pokemon.editors.Gen5EditorFrame editor = new com.dabomstew.pkrandom.pokemon.editors.Gen5EditorFrame(
                        romHandler);
                editor.setVisible(true);
            }
        });
    ...
    private void openGen6Editor() {
        if (romHandler == null) {
            JOptionPane.showMessageDialog(frame,
                    "No ROM loaded. Please load a Gen 6 ROM first.",
                    "Error",
                    JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        SwingUtilities.invokeLater(new Runnable() {
            @Override
            public void run() {
                com.dabomstew.pkrandom.pokemon.editors.Gen6EditorFrame editor = new com.dabomstew.pkrandom.pokemon.editors.Gen6EditorFrame(
                        romHandler);
                editor.setVisible(true);
            }
        });
    ...
    private void openGen7Editor() {
        if (romHandler == null) {
            JOptionPane.showMessageDialog(frame,
                    "No ROM loaded. Please load a Gen 7 ROM first.",
                    "Error",
                    JOptionPane.ERROR_MESSAGE);
            return;
        }

        if (!(romHandler instanceof Gen7RomHandler)) {
            JOptionPane.showMessageDialog(frame,
                    "The loaded ROM is not a Generation 7 title.",
                    "Unsupported ROM",
                    JOptionPane.WARNING_MESSAGE);
            return;
        }
        ...
        SwingUtilities.invokeLater(new Runnable() {
            @Override
            public void run() {
                com.dabomstew.pkrandom.pokemon.editors.Gen7EditorFrame editor = new com.dabomstew.pkrandom.pokemon.editors.Gen7EditorFrame(
                        romHandler);
                editor.setVisible(true);
            }
        });
    ...
    private void populateDropdowns() {
        ...
        List<Species> allPokes = romHandler.generationOfPokemon() >= 6 ? romHandler.getSpeciesInclFormes()
                .stream()
                .filter(pk -> pk == null || !pk.isCosmeticReplacement())
                .collect(Collectors.toList()) : romHandler.getSpecies();
        ...
        tpBattleStyleCombobox
                .setModel(new DefaultComboBoxModel<>(getBattleStylesForGeneration(romHandler.generationOfPokemon())));
        tpBattleStyleCombobox.setRenderer(new DefaultListCellRenderer() {
            ...
            public Component getListCellRendererComponent(JList list, Object value, int index, boolean isSelected,
                ...
                if (index >= 0 && value != null) {
                    list.setToolTipText(bundle
                            .getString(selectableBattleStylesTooltips.get(selectableBattleStyles.indexOf(value))));
                }
                return comp;
            }
        });
