...
public class Gen3RomHandler extends AbstractGBRomHandler {
    ...
    private static final Map<String, Integer> TYPE_EFFECTIVENESS_POINTERS = initTypeEffectivenessPointerMap();
    ...
    protected void initRomEntry() {
        ...
        updateTypeEffectivenessOffsetFromPointer();
    }
    ...
    private void addMapIconInfoToRomEntry() {
        ...
        private void updateTypeEffectivenessOffsetFromPointer() {
            if (romEntry == null) {
                return;
            }
            ...
            int tableOffset = readPointer(pointerLocation, true);
            if (tableOffset >= 0) {
                romEntry.putIntValue("TypeEffectivenessOffset", tableOffset);
            }
        }
    ...
    private TypeTable readTypeTable() {
        ...
        while (attackingType != GBConstants.typeTableTerminator) {
            if (rom[currentOffset] != GBConstants.typeTableForesightTerminator) {
                ...
                // This prevents crashes when reopening ROMs that have been modified
                ...
                // If types are unrecognized (e.g., custom types from other editors), skip silently
            }
            currentOffset += 3;
    ...
    private void writeTypeTable(TypeTable typeTable) {
        ...
        int originalLength = getTypeEffectivenessTableLength(currentOffset);
        int writeOffset = currentOffset;
        boolean repointed = false;

        if (requiredLength > originalLength) {
            writeOffset = findAndUnfreeSpace(requiredLength);
            repointed = true;
        }

        writeTypeTableParts(writeOffset, mainPart, ghostImmunities);

        if (repointed) {
            byte[] oldPointerBytes = pointerBytesForOffset(currentOffset);
            byte[] newPointerBytes = pointerBytesForOffset(writeOffset);
            int replacements = replacePointerBytes(oldPointerBytes, newPointerBytes);
            if (replacements == 0) {
                freeSpace(writeOffset, requiredLength);
                throw new RomIOException("Failed to locate Type Effectiveness pointer for repointing.");
            }
            freeSpace(currentOffset, originalLength);
            romEntry.putIntValue("TypeEffectivenessOffset", writeOffset);
        } else if (requiredLength < originalLength) {
            freeSpace(writeOffset + requiredLength, originalLength - requiredLength);
        }
    }

    private int getTypeEffectivenessTableLength(int offset) {
        int cursor = offset;
        while (true) {
            if (cursor + 2 >= rom.length) {
                throw new RomIOException("Type Effectiveness table not terminated before ROM end.");
            }
            if (rom[cursor] == GBConstants.typeTableTerminator
                    && rom[cursor + 1] == GBConstants.typeTableTerminator
                    && rom[cursor + 2] == 0x00) {
    ...
    private byte[] pointerBytesForOffset(int offset) {
        int pointer = offset + 0x08000000;
        return new byte[] {
                (byte) (pointer & 0xFF),
                (byte) ((pointer >> 8) & 0xFF),
