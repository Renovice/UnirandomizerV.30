...
public class Gen5RomHandler extends AbstractDSRomHandler {
    ...
    private static final int STANDARD_TYPE_COUNT = 17;
    private static final int FAIRY_TYPE_COUNT = 18;
    private static final int DEFAULT_BW1_FAIRY_TABLE_OFFSET = 0x1F80;
    ...
    private void loadMoves() {
        try {
            ...
            for (int i = 1; i <= Gen5Constants.moveCount; i++) {
                ...
                // Use byteToType to handle both 17-type and 18-type (Fairy) ROMs
                move.type = Gen5Constants.byteToType(moveData[0], hasFairyType);
                move.priority = (byte) moveData[6];
                move.effectIndex = readWord(moveData, 16);

                int categoryQuality = moveData[1] & 0xFF;
                move.categoryQuality = categoryQuality;

                int damageType = moveData[2] & 0xFF;
                if (damageType >= 0 && damageType < Gen5Constants.moveCategoryIndices.length) {
                    move.category = Gen5Constants.moveCategoryIndices[damageType];
                } else {
                    move.category = MoveCategory.STATUS;
                }

                int multiHitPacked = moveData[7] & 0xFF;
                move.minHits = multiHitPacked & 0x0F;
                move.maxHits = (multiHitPacked >> 4) & 0x0F;
                if (move.minHits == 0 && move.maxHits == 0) {
                    move.minHits = 1;
                    move.maxHits = 1;
                }

                move.minTrapTurns = moveData[12] & 0xFF;
                move.maxTrapTurns = moveData[13] & 0xFF;

                int rawStatusEffect = readWord(moveData, 8);
                move.statusEffect = rawStatusEffect;

                int statusChance = moveData[10] & 0xFF;
                move.statusPercentChance = statusChance;
                move.secondaryEffectChance = statusChance;

                int critStage = moveData[14] & 0xFF;
                move.criticalStage = critStage;
                if (critStage >= 6) {
                ...
                int leechRecoil = (byte) moveData[18];
                move.recoilPercent = leechRecoil;
                move.absorbPercent = moveData[19] & 0xFF;
                move.target = moveData[20] & 0xFF;

                int flags = (moveData[32] & 0xFF) | ((moveData[33] & 0xFF) << 8);
                move.makesContact = (flags & 0x0001) != 0;
                move.isChargeMove = (flags & 0x0002) != 0;
                move.isRechargeMove = (flags & 0x0004) != 0;
                move.isProtectedFromProtect = (flags & 0x0008) != 0;
                move.isMagicCoatAffected = (flags & 0x0010) != 0;
                move.isSnatchAffected = (flags & 0x0020) != 0;
                move.isMirrorMoveAffected = (flags & 0x0040) != 0;
                move.isPunchMove = (flags & 0x0080) != 0;
                move.isSoundMove = (flags & 0x0100) != 0;
                move.groundedByGravity = (flags & 0x0200) != 0;
                move.defrostsUser = (flags & 0x0400) != 0;
                move.hitsNonAdjacent = (flags & 0x0800) != 0;
                move.isHealingMove = (flags & 0x1000) != 0;
                move.bypassesSubstitute = (flags & 0x2000) != 0;
                move.extraFlag1 = (flags & 0x4000) != 0;
                move.extraFlag2 = (flags & 0x8000) != 0;

                if (i == MoveIDs.swift) {
                    perfectAccuracy = (int) move.hitratio;
                }
                ...
                } else if (GlobalConstants.normalMultihitMoves.contains(i)) {
                    move.hitCount = 19 / 6.0;
                } else if (GlobalConstants.doubleHitMoves.contains(i)) {
                    move.hitCount = 2;
                } else if (i == MoveIDs.tripleKick) {
                    move.hitCount = 2.71; // this assumes the first hit lands
                }

                switch (categoryQuality) {
                ...
                for (int statChange = 0; statChange < 3; statChange++) {
                    int statType = moveData[21 + statChange] & 0xFF;
                    move.statChanges[statChange].type = statType < StatChangeType.values().length
                            ? StatChangeType.values()[statType]
                            : StatChangeType.NONE;
                    move.statChanges[statChange].stages = (byte) moveData[24 + statChange];
                    move.statChanges[statChange].percentChance = moveData[27 + statChange] & 0xFF;
                }

                move.isTrapMove = move.minTrapTurns > 0 || move.maxTrapTurns > 0
                        || move.effectIndex == Gen5Constants.trappingEffect || rawStatusEffect == 8;

                if (rawStatusEffect >= 0 && rawStatusEffect < StatusType.values().length) {
                    move.statusType = StatusType.values()[rawStatusEffect];
                    if (move.statusType == StatusType.POISON && (i == MoveIDs.toxic || i == MoveIDs.poisonFang)) {
                    ...
                    move.statusType = StatusType.NONE;
                }
            }
        } catch (IOException e) {
            throw new RomIOException(e);
    ...
    private void loadBasicPokeStats(Species pkmn, byte[] stats, Map<Integer,FormeInfo> altFormes) {
        ...
        // Type - use byteToType to handle both 17-type and 18-type (Fairy) ROMs
        pkmn.setPrimaryType(Gen5Constants.byteToType(stats[Gen5Constants.bsPrimaryTypeOffset], hasFairyType));
        pkmn.setSecondaryType(Gen5Constants.byteToType(stats[Gen5Constants.bsSecondaryTypeOffset], hasFairyType));
        // Only one type?
        ...
        // EV Yields - packed into bytes 10-11 with bit masking
        int evByte1 = stats[Gen5Constants.bsEVYieldOffset] & 0xFF;
        int evByte2 = stats[Gen5Constants.bsEVYieldOffset + 1] & 0xFF;
        pkmn.setHpEvYield(evByte1 & 0b11);
        pkmn.setAttackEvYield((evByte1 & 0b1100) >> 2);
        pkmn.setDefenseEvYield((evByte1 & 0b110000) >> 4);
        pkmn.setSpeedEvYield((evByte1 & 0b11000000) >> 6);
        pkmn.setSpatkEvYield(evByte2 & 0b11);
        pkmn.setSpdefEvYield((evByte2 & 0b1100) >> 2);

        // Additional Pokemon data
        pkmn.setHatchCounter(stats[Gen5Constants.bsHatchCounterOffset] & 0xFF);
        pkmn.setBaseHappiness(stats[Gen5Constants.bsBaseHappinessOffset] & 0xFF);
        pkmn.setEggGroup1(stats[Gen5Constants.bsEggGroup1Offset] & 0xFF);
        pkmn.setEggGroup2(stats[Gen5Constants.bsEggGroup2Offset] & 0xFF);
        pkmn.setColor(stats[Gen5Constants.bsPokedexColorOffset] & 0xFF);
        pkmn.setExpYield(readWord(stats, Gen5Constants.bsExpYieldOffset));
    ...
    public void saveMoves() {
        // Ensure hasFairyType is initialized by reading type table first
        if (typeTable == null) {
            getTypeTable();
        }
        for (int i = 1; i <= Gen5Constants.moveCount; i++) {
            ...
            int minHits = clamp(move.minHits, 0, 15);
            int maxHits = clamp(move.maxHits, 0, 15);
            data[7] = (byte) ((maxHits << 4) | minHits);

            writeWord(data, 8, clamp(move.statusEffect, 0, 0xFFFF));
            data[10] = (byte) clamp((int) Math.round(move.statusPercentChance), 0, 255);
            data[12] = (byte) clamp(move.minTrapTurns, 0, 255);
            data[13] = (byte) clamp(move.maxTrapTurns, 0, 255);
            ...
            for (int statChange = 0; statChange < 3; statChange++) {
                Move.StatChange sc = move.statChanges[statChange];
                if (sc == null) {
                    sc = new Move.StatChange();
                    move.statChanges[statChange] = sc;
                }
                int statType = sc.type != null ? sc.type.ordinal() : 0;
                if (statType < 0 || statType >= StatChangeType.values().length) {
                    statType = 0;
                }
                data[21 + statChange] = (byte) statType;
                data[24 + statChange] = (byte) clampSigned(sc.stages, -128, 127);
                data[27 + statChange] = (byte) clamp((int) Math.round(sc.percentChance), 0, 255);
            }
            ...
            if (move.groundedByGravity) {
                flags |= 0x0200;
            }
            if (move.defrostsUser) {
                flags |= 0x0400;
            }
            if (move.hitsNonAdjacent) {
                flags |= 0x0800;
            }
            if (move.isHealingMove) {
                flags |= 0x1000;
            }
            if (move.bypassesSubstitute) {
                flags |= 0x2000;
            }
            if (move.extraFlag1) {
                flags |= 0x4000;
    ...
    private void saveBasicPokeStats(Species pkmn, byte[] stats) {
        ...
        stats[Gen5Constants.bsPrimaryTypeOffset] = Gen5Constants.typeToByte(pkmn.getPrimaryType(false), hasFairyType);
        if (pkmn.getSecondaryType(false) == null) {
            stats[Gen5Constants.bsSecondaryTypeOffset] = stats[Gen5Constants.bsPrimaryTypeOffset];
        } else {
            stats[Gen5Constants.bsSecondaryTypeOffset] = Gen5Constants.typeToByte(pkmn.getSecondaryType(false), hasFairyType);
        }
        ...
        // EV Yields - pack into bytes 10-11 with bit masking
        int evByte1 = (pkmn.getHpEvYield() & 0b11) |
                     ((pkmn.getAttackEvYield() & 0b11) << 2) |
                     ((pkmn.getDefenseEvYield() & 0b11) << 4) |
                     ((pkmn.getSpeedEvYield() & 0b11) << 6);
        int evByte2 = (pkmn.getSpatkEvYield() & 0b11) |
                     ((pkmn.getSpdefEvYield() & 0b11) << 2);
        stats[Gen5Constants.bsEVYieldOffset] = (byte) evByte1;
        stats[Gen5Constants.bsEVYieldOffset + 1] = (byte) evByte2;

        // Additional Pokemon data
        stats[Gen5Constants.bsHatchCounterOffset] = (byte) pkmn.getHatchCounter();
        stats[Gen5Constants.bsBaseHappinessOffset] = (byte) pkmn.getBaseHappiness();
        stats[Gen5Constants.bsEggGroup1Offset] = (byte) pkmn.getEggGroup1();
        stats[Gen5Constants.bsEggGroup2Offset] = (byte) pkmn.getEggGroup2();
        stats[Gen5Constants.bsPokedexColorOffset] = (byte) pkmn.getColor();
        writeWord(stats, Gen5Constants.bsExpYieldOffset, pkmn.getExpYield());
...
//            }
        ...
        try {
            ...
            for (int i = 1; i < trainernum; i++) {
                ...
                for (int poke = 0; poke < numPokes; poke++) {
                    ...
                    int species = sanitizeTrainerSpeciesId(readWord(trpoke, pokeOffs + 4), tr, poke);
    ...
    private int sanitizeTrainerSpeciesId(int rawSpeciesId, Trainer trainer, int pokemonIndex) {
        if (rawSpeciesId > 0 && rawSpeciesId < pokes.length && pokes[rawSpeciesId] != null) {
            return rawSpeciesId;
        }

        int fallback = SpeciesIDs.bulbasaur;
        if (fallback >= pokes.length || pokes[fallback] == null) {
            fallback = Math.min(Math.max(1, rawSpeciesId % Math.max(1, pokes.length)), pokes.length - 1);
        }

        System.out.println("WARNING: Trainer " + trainer.index + " (" + trainer.fullDisplayName + ") Pokemon #"
                + (pokemonIndex + 1) + " references invalid species id " + rawSpeciesId
                + ". Using fallback id " + fallback + " instead.");
        return fallback;
    }
    ...
    public void setEggMoves(Map<Integer, List<Integer>> eggMoves) {
        try {
            ...
            for (int i = 1; i <= Gen5Constants.pokemonCount; i++) {
                ...
                int maxSlots = movedata.length > 2 ? (movedata.length - 2) / 2 : 0;

                if (moveCount > maxSlots) {
                    // Allocate a fresh buffer so species that originally had no slots can store new moves.
                    byte[] expanded = new byte[2 + moveCount * 2];
                    writeWord(expanded, 0, moveCount);
                    for (int j = 0; j < moveCount; j++) {
                        Integer moveValue = moves.get(j);
                        int moveId = (moveValue == null || moveValue < 0) ? 0 : moveValue;
                        writeWord(expanded, 2 + (j * 2), moveId);
                    }
                    eggMovesNarc.files.set(i, expanded);
                    continue;
                }

                writeWord(movedata, 0, moveCount);
                for (int j = 0; j < moveCount; j++) {
                    Integer moveValue = moves.get(j);
                    int moveId = (moveValue == null || moveValue < 0) ? 0 : moveValue;
                    writeWord(movedata, 2 + (j * 2), moveId);
                }
                for (int j = moveCount; j < maxSlots; j++) {
                    writeWord(movedata, 2 + (j * 2), 0);
                }
            }
            // Save
    ...
    private static class TypeTableLoadResult {
        final TypeTable table;
        final int offset;
        final boolean inArm9;

        TypeTableLoadResult(TypeTable table, int offset, boolean inArm9) {
            this.table = table;
            this.offset = offset;
            this.inArm9 = inArm9;
        }
    ...
    private byte[] loadBattleOverlay() throws IOException {
        return readOverlay(romEntry.getIntValue("BattleOvlNumber"));
    }

    private static class TypeTableBuffer {
        final byte[] data;
        final boolean fromArm9;
        final int offset;

        TypeTableBuffer(byte[] data, boolean fromArm9, int offset) {
            this.data = data;
            this.fromArm9 = fromArm9;
            this.offset = offset;
        }
    }

    private TypeTableBuffer resolveTypeTableFromPointer(int tableWidth) throws IOException {
        int pointerLocation = romEntry.getIntValue("TypeEffectivenessOffset");
        int overlayNumber = romEntry.getIntValue("BattleOvlNumber");
        byte[] battleOverlay = loadBattleOverlay();
        int pointerValue = readLong(battleOverlay, pointerLocation);
        int overlayBase = overlayAddress(overlayNumber);
        int overlayOffset = pointerValue - overlayBase;
        int requiredSize = tableWidth * tableWidth;

        if (DEBUG_TYPE_TABLE) {
            System.out.println(String.format("DEBUG: Pointer lookup -> pointerLocation=0x%X pointerValue=0x%X overlayBase=0x%X overlayOffset=0x%X",
                    pointerLocation, pointerValue, overlayBase, overlayOffset));
        }

        if (overlayOffset >= 0 && overlayOffset + requiredSize <= battleOverlay.length) {
        ...
        int arm9Offset = pointerValue - getARM9Offset();
        if (arm9Offset >= 0 && arm9Offset + requiredSize <= arm9.length) {
    ...
    private TypeTableBuffer resolveTypeTableHeuristically(int preferredWidth) throws IOException {
        int configuredOffset = romEntry.getIntValue("TypeEffectivenessOffset");
        int[] widthsToTry = preferredWidth == STANDARD_TYPE_COUNT
                ? new int[]{STANDARD_TYPE_COUNT}
                : new int[]{preferredWidth, STANDARD_TYPE_COUNT};

        byte[] battleOverlay = loadBattleOverlay();
        for (int width : widthsToTry) {
            if (looksLikeTypeTable(battleOverlay, configuredOffset, width)) {
                typeEffectivenessStoredInArm9 = false;
                resolvedTypeEffectivenessOffset = configuredOffset;
                return new TypeTableBuffer(battleOverlay, false, configuredOffset);
            }
        }

        if (arm9 != null) {
            for (int width : widthsToTry) {
                if (looksLikeTypeTable(arm9, configuredOffset, width)) {
                    typeEffectivenessStoredInArm9 = true;
                    resolvedTypeEffectivenessOffset = configuredOffset;
                    return new TypeTableBuffer(arm9, true, configuredOffset);
                }
            }
        }

        for (int width : widthsToTry) {
            int candidate = findGenericTypeTableOffset(battleOverlay, width);
            if (candidate != -1) {
                typeEffectivenessStoredInArm9 = false;
                resolvedTypeEffectivenessOffset = candidate;
                return new TypeTableBuffer(battleOverlay, false, candidate);
            }
            if (arm9 != null) {
                candidate = findGenericTypeTableOffset(arm9, width);
                if (candidate != -1) {
                    typeEffectivenessStoredInArm9 = true;
                    resolvedTypeEffectivenessOffset = candidate;
                    return new TypeTableBuffer(arm9, true, candidate);
                }
            }
        }

        if (hasSpaceForTable(battleOverlay, configuredOffset, STANDARD_TYPE_COUNT)) {
            typeEffectivenessStoredInArm9 = false;
            resolvedTypeEffectivenessOffset = configuredOffset;
            return new TypeTableBuffer(battleOverlay, false, configuredOffset);
        }

        if (hasSpaceForTable(arm9, configuredOffset, STANDARD_TYPE_COUNT)) {
            typeEffectivenessStoredInArm9 = true;
            resolvedTypeEffectivenessOffset = configuredOffset;
            return new TypeTableBuffer(arm9, true, configuredOffset);
        }

        throw new IllegalStateException("Type effectiveness offset " + configuredOffset
                + " is outside both battle overlay " + romEntry.getIntValue("BattleOvlNumber") + " and ARM9");
    }

    private TypeTableBuffer locateTypeTableBuffer(int preferredWidth) throws IOException {
        if (resolvedTypeEffectivenessOffset >= 0 && typeEffectivenessStoredInArm9 != null) {
            byte[] data = Boolean.TRUE.equals(typeEffectivenessStoredInArm9) ? arm9 : loadBattleOverlay();
            TypeTableBuffer buffer = new TypeTableBuffer(data,
                    Boolean.TRUE.equals(typeEffectivenessStoredInArm9), resolvedTypeEffectivenessOffset);
            debugTypeTableChoice(buffer, preferredWidth, "cached");
            return buffer;
        }

        TypeTableBuffer pointerResolved = resolveTypeTableFromPointer(preferredWidth);
        if (pointerResolved != null) {
            debugTypeTableChoice(pointerResolved, preferredWidth, "pointer");
            return pointerResolved;
        }

        TypeTableBuffer heuristic = resolveTypeTableHeuristically(preferredWidth);
        debugTypeTableChoice(heuristic, preferredWidth, "heuristic");
        return heuristic;
    }

    private void debugTypeTableChoice(TypeTableBuffer buffer, int width, String method) {
        if (!DEBUG_TYPE_TABLE || buffer == null) {
            return;
        }
    ...
    private void dumpTypeTablePreview(byte[] data, int baseOffset, int width) {
        ...
        for (int row = 0; row < rowsToShow; row++) {
            ...
            for (int col = 0; col < width; col++) {
                int val = data[baseOffset + row * width + col] & 0xFF;
                sb.append(String.format("%02X ", val));
            }
            System.out.println("  Row " + row + ": " + sb);
        }
    }
    private boolean hasSpaceForTable(byte[] data, int offset, int tableWidth) {
        return data != null && offset >= 0 && offset + tableWidth * tableWidth <= data.length;
    }

    private boolean looksLikeTypeTable(byte[] data, int offset, int tableWidth) {
        if (!hasSpaceForTable(data, offset, tableWidth)) {
            return false;
        }
        int total = tableWidth * tableWidth;
        int valid = 0;
        ...
        if (valid < (int) (total * 0.9)) {
            return false;
        }
        for (int i = 0; i < tableWidth; i++) {
            if ((data[offset + i * tableWidth + i] & 0xFF) != 4) {
                return false;
            }
        }
        return true;
    ...
    /**
     * Attempts to find the actual type effectiveness table offset for Fairy-modded ROMs.
     * Some ROM hacks move or reorganize the type table, so we search for it by looking
     * for the characteristic pattern of a valid 18x18 type effectiveness table.
     *
     * @param battleOverlay The battle overlay data
     * @param vanillaOffset The vanilla type table offset
    ...
    private int findFairyTypeTableOffset(byte[] data, int vanillaOffset, String sourceLabel) {
        ...
        // Try multiple locations where fairy patches might place the type table
        int[] offsets = {
            0x0,            // b2w2-fairy-master location (patch places table at start of overlay)
            vanillaOffset   // Vanilla location (some fairy hacks extend the vanilla table in-place)
        };

        if (romEntry.getRomType() == Gen5Constants.Type_BW2
                && !Boolean.TRUE.equals(typeEffectivenessStoredInArm9)
                && sourceLabel.contains("overlay")) {
            offsets = mergeSpecialBW2Offsets(offsets);
        }

        System.out.println("\nSearching for 18x18 type table...");
        ...
        for (int testOffset : offsets) {
            System.out.println("Checking offset 0x" + Integer.toHexString(testOffset) + "...");
            dumpTypeTablePreview(data, testOffset, FAIRY_TYPE_COUNT);

            if (isValidTypeTable18x18(data, testOffset)) {
                System.out.println("SUCCESS: Found valid 18x18 type table at offset 0x" + Integer.toHexString(testOffset));
                return testOffset;
            }
        }

        System.out.println("WARNING: No valid 18x18 type table found at expected offsets!");
        System.out.println("Tried: 0x0 (b2w2-fairy), 0x" + Integer.toHexString(vanillaOffset) + " (vanilla extended)");

        // Fallback: search for the table elsewhere (shouldn't be needed for proper fairy ROMs)
        System.out.println("\nFallback: Searching for 18x18 table in battle overlay...");
        int searchStart = Math.min(4096, Math.max(0, data.length - 324));
        for (int candidate = searchStart; candidate <= data.length - 324; candidate += 4) {
            if (isValidTypeTable18x18(data, candidate)) {
                System.out.println("Found valid 18x18 type table at offset " + candidate + ".");
                return candidate;
            }
        }

        // DIAGNOSTIC: Dump first 3 rows at vanilla offset to see what's there
        System.out.println("\nDIAGNOSTIC: First 54 bytes (3 rows of 18) at vanilla offset:");
        for (int row = 0; row < 3; row++) {
            StringBuilder rowData = new StringBuilder("Row " + row + ": ");
            for (int col = 0; col < 18; col++) {
                int offset = vanillaOffset + (row * 18) + col;
                if (offset < data.length) {
                    rowData.append(String.format("%02X ", data[offset] & 0xFF));
                }
            }
            System.out.println(rowData.toString());
        }

        // Check if vanilla 17x17 table is present
        System.out.println("\nChecking for vanilla 17x17 table...");
        boolean has17x17 = checkVanilla17x17Table(data, vanillaOffset);
        System.out.println("Valid 17x17 table at vanilla offset: " + has17x17);

        // Search strategies
        int searchRange = 50 * 1024;
        int searchEnd = Math.min(data.length - 324, vanillaOffset + searchRange);
        searchStart = Math.max(0, vanillaOffset - searchRange);

        System.out.println("\nPhase 1: Searching near vanilla offset (" + searchStart + " to " + searchEnd + ")");

        for (int testOffset = searchStart; testOffset < searchEnd; testOffset++) {
            if (isValidTypeTable18x18(data, testOffset)) {
                return testOffset;
            }
        }

        int fairyBasedOffset = findTableByFairyInteractions(data, searchStart, searchEnd, sourceLabel);
        if (fairyBasedOffset != -1) {
            return fairyBasedOffset;
        }

        System.out.println("\nPhase 2: Full search of " + sourceLabel + " (may take 10-30 seconds)...");
        fairyBasedOffset = findTableByFairyInteractions(data, 0, data.length - 324, sourceLabel);
        if (fairyBasedOffset != -1) {
            return fairyBasedOffset;
        }

        int genericOffset = findGenericTypeTableOffset(data, FAIRY_TYPE_COUNT);
        if (genericOffset != -1) {
            System.out.println("Generic pattern search found a potential 18x18 type table at offset " + genericOffset
                    + " in " + sourceLabel + ". Using that as the Fairy effectiveness matrix.");
            return genericOffset;
        }

        System.out.println("\n*** No 18x18 type table found ***");
        System.out.println("The ROM might use:");
        System.out.println("  - Repointers to external data");
        System.out.println("  - Modified 17x17 table with Fairy handled elsewhere");
        System.out.println("  - Different overlay for Fairy type");
        return -1;
    }

    private int findSignatureFairyTableOffset(byte[] data) {
        if (data == null || data.length < B2W2_FAIRY_TABLE.length) {
            return -1;
        }
        for (int offset = 0; offset <= data.length - B2W2_FAIRY_TABLE.length; offset++) {
    ...
    private int[] mergeSpecialBW2Offsets(int[] baseOffsets) {
        ...
        for (int value : specialCandidates) {
            merged.add(value);
        }
        for (int value : baseOffsets) {
    ...
    private boolean checkVanilla17x17Table(byte[] data, int offset) {
        if (offset + 289 > data.length) return false;
        int validCount = 0;
        for (int i = 0; i < 289; i++) {
            int val = data[offset + i] & 0xFF;
            if (val == 0 || val == 2 || val == 4 || val == 8) validCount++;
        }
        if (validCount < 275) {
            return false;
        }
        for (int i = 0; i < 17; i++) {
            if ((data[offset + i * 17 + i] & 0xFF) != 4) {
                return false;
            }
        }
        return true;
    ...
    private int findGenericTypeTableOffset(byte[] data, int tableWidth) {
        if (data == null) {
            return -1;
        }
        int totalCells = tableWidth * tableWidth;
        for (int offset = 0; offset <= data.length - totalCells; offset++) {
            if (tableWidth == FAIRY_TYPE_COUNT) {
                if (isValidTypeTable18x18(data, offset)) {
                    return offset;
                }
            } else {
                if (checkVanilla17x17Table(data, offset)) {
                    return offset;
                }
            }
        }
    ...
    /**
     * Searches for the type table by looking for known Fairy type interactions.
     * Fairy is super effective against Dragon (0x0F), Dark (0x10), Fighting (0x01)
     * Fairy is weak to Poison (0x03) and Steel (0x08)
     * Fairy resists Fighting (0x01), Bug (0x06), Dark (0x10)
     */
    private int findTableByFairyInteractions(byte[] data, int searchStart, int searchEnd, String sourceLabel) {
        System.out.println("Searching for Fairy type interactions in " + sourceLabel + " from " + searchStart + " to " + searchEnd + "...");

        int checked = 0;
        for (int testOffset = searchStart; testOffset < searchEnd; testOffset++) {
            if (testOffset + 324 > data.length) break;

            checked++;
            if (checked % 10000 == 0) {
                System.out.println("  Checked " + checked + " offsets...");
            }

            // Check if this looks like a Fairy row in an 18x18 table
            // Fairy is type 0x11 (index 17), so its row starts at offset + (17 * 18)
            // But the table might be reordered, so we'll check all possible row positions

            for (int fairyRow = 0; fairyRow < 18; fairyRow++) {
                int rowOffset = testOffset + (fairyRow * 18);

                if (checkFairyAttackingRow(data, rowOffset)) {
                    // Found a potential Fairy attacking row, now verify Fairy defending column
                    if (checkFairyDefendingColumn(data, testOffset, fairyRow)) {
                        System.out.println("Found Fairy type table at offset " + testOffset +
                                         " in " + sourceLabel + " with Fairy at row/col index " + fairyRow);
                        return testOffset;
                    }
                }
            }
        ...
        System.out.println("  Checked " + checked + " total offsets, no Fairy table found");
        return -1;
    }

    /**
     * Checks if a row matches Fairy's attacking type chart.
     * Fairy is super effective (8) against Dragon, Dark, Fighting
     * Fairy has no immunities
     */
    private boolean checkFairyAttackingRow(byte[] data, int rowOffset) {
        if (rowOffset + 18 > data.length) return false;

        int superEffectiveCount = 0;
        int neutralCount = 0;

        for (int i = 0; i < 18; i++) {
            int val = data[rowOffset + i] & 0xFF;

            // All values must be valid effectiveness values
            if (val != 0 && val != 2 && val != 4 && val != 8) {
                return false;
            }

            if (val == 8) superEffectiveCount++;
            if (val == 4) neutralCount++;
        }

        // Fairy should be super effective against exactly 3 types (Dragon, Dark, Fighting)
        // and have no immunities (no 0 values from Fairy attacking)
        boolean hasNoImmunities = true;
        for (int i = 0; i < 18; i++) {
            if ((data[rowOffset + i] & 0xFF) == 0) {
                hasNoImmunities = false;
                break;
            }
        }

        return superEffectiveCount == 3 && neutralCount >= 14 && hasNoImmunities;
    }

    /**
     * Checks if a column matches Fairy's defending type chart.
     * Fairy is weak to (takes double damage from) Poison and Steel
     * Fairy resists Fighting, Bug, Dark
     * Fairy is immune to Dragon
     */
    private boolean checkFairyDefendingColumn(byte[] data, int tableOffset, int fairyIndex) {
        if (tableOffset + (17 * 18) + fairyIndex >= data.length) return false;

        int weakToCount = 0;      // Should be 2 (Poison, Steel)
        int resistsCount = 0;      // Should be 3 (Fighting, Bug, Dark)
        int immuneCount = 0;       // Should be 1 (Dragon)

        for (int attackerRow = 0; attackerRow < 18; attackerRow++) {
            int cellOffset = tableOffset + (attackerRow * 18) + fairyIndex;
            if (cellOffset >= data.length) return false;

            int val = data[cellOffset] & 0xFF;

            if (val == 8) weakToCount++;      // Double damage
            if (val == 2) resistsCount++;     // Half damage
            if (val == 0) immuneCount++;      // Immune
        }

        // Fairy should be weak to 2 types, resist 3 types, and be immune to 1 type
        return weakToCount == 2 && resistsCount == 3 && immuneCount == 1;
    }

    private boolean matchesB2W2FairyTable(byte[] data, int offset) {
        if (data == null || offset < 0 || offset + B2W2_FAIRY_TABLE.length > data.length) {
            return false;
        }
        for (int i = 0; i < B2W2_FAIRY_TABLE.length; i++) {
            if ((data[offset + i] & 0xFF) != (B2W2_FAIRY_TABLE[i] & 0xFF)) {
                return false;
            }
        }
        return true;
    ...
    /**
     * Checks if the data at the given offset looks like a valid 18x18 type effectiveness table.
     * Uses PURELY structural validation - checks format, not content.
     * This allows ANY randomization pattern to work.
     */
    private boolean isValidTypeTable18x18(byte[] data, int offset) {
        // Check if we have enough space for an 18x18 table (324 bytes)
        if (offset + 324 > data.length) {
            return false;
        }
        ...
        // A valid type effectiveness table has exactly 324 bytes (18x18)
        // where each byte is one of: 0 (immune), 2 (not very effective), 4 (neutral), 8 (super effective)

        int totalCells = 18 * 18; // 324 cells
        int validCells = 0;

        // Check that ALL 324 bytes are valid type effectiveness values
        for (int i = 0; i < totalCells; i++) {
            int val = data[offset + i] & 0xFF;
            if (val == 0 || val == 2 || val == 4 || val == 8) {
                validCells++;
            } else {
                // Found an invalid value - this is not a type table
                return false;
            }
        }
    ...
    /**
     * Detects if this Gen 5 ROM has been modified to include Fairy type.
     * ROM hacks like Blaze Black 2 Redux add Fairy type at byte position 0x11.
     *
     * This method uses the same approach as Frost's Gen 5 Editor:
     * Read the type name strings from the text NARC and check if "Fairy" is in the list.
     * This is much more reliable than trying to probe the binary type effectiveness table.
     *
    ...
    private boolean detectFairyType() {
        ...
        try {
            ...
            // Check if "Fairy" exists in the type names
            boolean hasFairy = false;
            for (int i = 0; i < typeNames.size(); i++) {
    ...
    private TypeTable readTypeTable() {
        try {
            ...
            if (!hasFairyType) {
                ...
                typeEffectivenessStoredInArm9 = false;
                resolvedTypeEffectivenessOffset = tableOffset;
                return readStandardTypeTable(battleOverlay, tableOffset);
            }

            // FAIRY ROM DETECTED - Everything below here is UNCHANGED from original
            TypeTableBuffer tableBuffer = locateTypeTableBuffer(hasFairyType ? FAIRY_TYPE_COUNT : STANDARD_TYPE_COUNT);
            byte[] primaryData = tableBuffer.data;

            if (hasFairyType) {
                TypeTableLoadResult fairyResult = tryConfiguredFairyTableOverride();
                Exception lastFairyException = null;

                if (fairyResult == null) {
                    try {
                        fairyResult = tryReadFairyTypeTable(primaryData, tableBuffer.offset,
                                tableBuffer.fromArm9, true);
                    } catch (Exception e) {
                        lastFairyException = e;
                    }
                }

                if (fairyResult == null) {
                    try {
                        TypeTableBuffer fallbackBuffer = resolveTypeTableHeuristically(FAIRY_TYPE_COUNT);
                        tableBuffer = fallbackBuffer;
                        primaryData = fallbackBuffer.data;
                        fairyResult = tryReadFairyTypeTable(fallbackBuffer.data, fallbackBuffer.offset, fallbackBuffer.fromArm9, true);
                    } catch (Exception fallbackException) {
                        lastFairyException = fallbackException;
                    }
                }

                if (fairyResult != null) {
                    hasFairyTypeTable = true;
                    typeEffectivenessStoredInArm9 = fairyResult.inArm9;
                    resolvedTypeEffectivenessOffset = fairyResult.offset;
                    System.out.println("Successfully loaded 18x18 type effectiveness table with Fairy type.");
                    return fairyResult.table;
                } else {
                    System.out.println("Warning: Fairy type detected in text file, but could not find valid 18x18 type effectiveness table.");
                    System.out.println("Falling back to 17x17 table. Fairy type will work for Pokemon/Move types, but not type effectiveness.");
                    hasFairyTypeTable = false;
                    if (lastFairyException != null) {
                        lastFairyException.printStackTrace(System.out);
                    }
                }
            }

            return readStandardTypeTable(primaryData, resolvedTypeEffectivenessOffset);
    ...
    private TypeTableLoadResult tryConfiguredFairyTableOverride() {
        int overrideOffset = romEntry.getIntValue("FairyTypeTableOffset");
        boolean overrideFromArm9 = romEntry.getIntValue("FairyTypeTableInArm9") == 1;
        int overrideOverlayNumber = romEntry.getIntValue("FairyTypeTableOverlayNumber");

        boolean usingDefaultOverride = false;
        if (overrideOffset <= 0) {
            boolean isBW2 = romEntry.getRomType() == Gen5Constants.Type_BW2;
            overrideOffset = isBW2 ? DEFAULT_BW2_FAIRY_TABLE_OFFSET : DEFAULT_BW1_FAIRY_TABLE_OFFSET;
            overrideOverlayNumber = isBW2 ? DEFAULT_BW2_FAIRY_TABLE_OVERLAY : DEFAULT_BW1_FAIRY_TABLE_OVERLAY;
            overrideFromArm9 = false;
        ...
        try {
            ...
            if (overrideFromArm9) {
                ...
                int overlayToRead = overrideOverlayNumber > 0 ? overrideOverlayNumber : romEntry.getIntValue("BattleOvlNumber");
                sourceData = (overlayToRead == romEntry.getIntValue("BattleOvlNumber"))
                        ? loadBattleOverlay()
                        : readOverlay(overlayToRead);
                fromArm9 = false;
            }

            if (!isValidTypeTable18x18(sourceData, overrideOffset)) {
                System.out.println("WARNING: FairyTypeTableOffset override at 0x"
                        + Integer.toHexString(overrideOffset) + " failed validation. Ignoring override.");
                return null;
            }
            ...
            System.out.println("DEBUG: Using configured Fairy type table offset 0x"
                    ...
                    + ")")
                    + (usingDefaultOverride ? " [default override]" : ""));
            return buildFairyTypeTable(sourceData, overrideOffset, fromArm9);
        } catch (IOException e) {
            throw new RomIOException(e);
        }
    }
    ...
    /**
     * Attempts to read an 18x18 type effectiveness table for Fairy-modded ROMs.
     * Throws exception if table cannot be found or is invalid.
     */
    private TypeTableLoadResult tryReadFairyTypeTable(byte[] data, int vanillaOffset, boolean dataFromArm9,
                                                      boolean allowCrossSourceRetry) throws IOException {
        if (data == null) {
            return null;
        }

        String sourceLabel = dataFromArm9 ? "ARM9" : "battle overlay " + romEntry.getIntValue("BattleOvlNumber");
        int tableOffset = findFairyTypeTableOffset(data, vanillaOffset, sourceLabel);

        if (tableOffset == -1) {
            if (allowCrossSourceRetry) {
                if (!dataFromArm9) {
                    System.out.println("\nType table not found in battle overlay. Checking ARM9 instead...");
                    return tryReadFairyTypeTable(arm9, romEntry.getIntValue("TypeEffectivenessOffset"), true, false);
                } else {
                    System.out.println("\nType table not found in ARM9 either. Trying battle overlay as last resort...");
                    byte[] battleOverlay = loadBattleOverlay();
                    return tryReadFairyTypeTable(battleOverlay, 0, false, false);
                }
            }
            return null;
        }

        return buildFairyTypeTable(data, tableOffset, dataFromArm9);
    }

    private TypeTableLoadResult buildFairyTypeTable(byte[] data, int tableOffset, boolean dataFromArm9) {
        List<Type> types = Type.GEN6PLUS; // 18 types including Fairy
        TypeTable typeTable = new TypeTable(types);
        int tableWidth = 18;

        for (Type attacker : types) {
            for (Type defender : types) {
                int offset = tableOffset + (Gen5Constants.typeToByte(attacker, true) * tableWidth)
                        + Gen5Constants.typeToByte(defender, true);
                int effectivenessInternal = data[offset] & 0xFF;
        ...
        TypeTableLoadResult result = new TypeTableLoadResult(typeTable, tableOffset, dataFromArm9);
        emitTypeTableDiag(result, data);
        return result;
    }

    private void emitTypeTableDiag(TypeTableLoadResult result, byte[] source) {
        try {
            long hash = FileFunctions.getCRC32(Arrays.copyOfRange(source, result.offset, result.offset + (FAIRY_TYPE_COUNT * FAIRY_TYPE_COUNT)));
            System.out.println(String.format(
                "INFO: Fairy type table located at 0x%X in %s (size=%d bytes, crc32=0x%08X)",
                result.offset,
                result.inArm9 ? "ARM9" : "overlay " + romEntry.getIntValue("BattleOvlNumber"),
                FAIRY_TYPE_COUNT * FAIRY_TYPE_COUNT,
                hash
            ));
        } catch (Exception ex) {
            System.out.println("INFO: Fairy type table located at 0x" + Integer.toHexString(result.offset)
                    + " (unable to compute CRC32: " + ex.getMessage() + ")");
        }
    }

    /**
     * Reads the standard 17x17 type effectiveness table.
     */
    private TypeTable readStandardTypeTable(byte[] typeTableData, int tableOffset) {
        List<Type> types = Type.GEN2THROUGH5; // 17 types
        TypeTable typeTable = new TypeTable(types);
        int tableWidth = 17;

        for (Type attacker : types) {
            for (Type defender : types) {
                int offset = tableOffset + (Gen5Constants.typeToByte(attacker, false) * tableWidth) + Gen5Constants.typeToByte(defender, false);
                int effectivenessInternal = typeTableData[offset] & 0xFF;
    ...
    private void writeTypeTable(TypeTable typeTable) {
        try {
            int tableWidth = typeTable.getTypes().size(); // 17 for vanilla, 18 for Fairy
            TypeTableBuffer buffer = locateTypeTableBuffer(tableWidth);

            byte[] targetData = buffer.data;
            int tableOffset = buffer.offset;

            // Use hasFairyTypeTable flag to determine if we should write an 18x18 table
            boolean writingFairyTable = (tableWidth == FAIRY_TYPE_COUNT && hasFairyTypeTable);

            for (Type attacker : typeTable.getTypes()) {
                for (Type defender : typeTable.getTypes()) {
                    // Use the overloaded method that handles Fairy type
                    int offset = tableOffset + (Gen5Constants.typeToByte(attacker, writingFairyTable) * tableWidth) + Gen5Constants.typeToByte(defender, writingFairyTable);
                    ...
                    targetData[offset] = effectivenessInternal;
                }
            }

            if (!buffer.fromArm9) {
                writeOverlay(romEntry.getIntValue("BattleOvlNumber"), targetData);
            }
        } catch (IOException e) {
            throw new RomIOException(e);
        }
    ...
    public boolean hasMoveTutors() {
        return romEntry.getRomType() == Gen5Constants.Type_BW
                || romEntry.getRomType() == Gen5Constants.Type_BW2;
    }

    @Override
    public List<Integer> getMoveTutorMoves() {
        if (romEntry.getRomType() == Gen5Constants.Type_BW) {
            List<Integer> moves = new ArrayList<>(Gen5Constants.bw1MoveTutorCount);
            for (int moveId : Gen5Constants.bw1MoveTutorMoveIds) {
                moves.add(moveId);
            }
            return moves;
        }
    ...
    public void setMoveTutorMoves(List<Integer> moves) {
        if (romEntry.getRomType() == Gen5Constants.Type_BW || !hasMoveTutors()) {
            return;
        }
        int baseOffset = romEntry.getIntValue("MoveTutorDataOffset");
        int amount = Gen5Constants.bw2MoveTutorCount;
    ...
    public Map<Species, boolean[]> getMoveTutorCompatibility() {
        ...
        if (romEntry.getRomType() == Gen5Constants.Type_BW) {
            ...
            int formeOffset = Gen5Constants.getFormeOffset(romEntry.getRomType());
            for (int i = 1; i <= Gen5Constants.pokemonCount + formeCount; i++) {
                byte[] data = (i > Gen5Constants.pokemonCount)
                        ? pokeNarc.files.get(i + formeOffset)
                        : pokeNarc.files.get(i);
                Species pkmn = pokes[i];
                boolean[] flags = new boolean[Gen5Constants.bw1MoveTutorCount + 1];
                readByteIntoFlags(data, flags, 1, Gen5Constants.bsMiscTutorsOffset);
                compat.put(pkmn, flags);
            }
            return compat;
        }
    ...
    public void setMoveTutorCompatibility(Map<Species, boolean[]> compatData) {
        ...
        if (romEntry.getRomType() == Gen5Constants.Type_BW) {
            ...
            for (Map.Entry<Species, boolean[]> compatEntry : compatData.entrySet()) {
                ...
                byte[] data = pokeNarc.files.get(number);
                data[Gen5Constants.bsMiscTutorsOffset] = getByteFromFlags(flags, 1);
            }
            return;
        }
        int formeOffset = Gen5Constants.getFormeOffset(romEntry.getRomType());
