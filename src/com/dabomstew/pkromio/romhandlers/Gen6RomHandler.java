...
public class Gen6RomHandler extends Abstract3DSRomHandler {
    ...
    private void loadItems() {
        ...
        // TODO: would some other system be better here; e.g. something similar to
        // "tagTrainers"
    ...
    private void loadBasicPokeStats(Species pkmn, byte[] stats, Map<Integer, FormeInfo> altFormes) {
        ...
        // EV Yields - packed in ushort at offset 0x0A
        int evYieldPacked = FileFunctions.read2ByteInt(stats, Gen6Constants.bsEVYieldOffset);
        pkmn.setHpEvYield(evYieldPacked & 0x3);
        pkmn.setAttackEvYield((evYieldPacked >> 2) & 0x3);
        pkmn.setDefenseEvYield((evYieldPacked >> 4) & 0x3);
        pkmn.setSpeedEvYield((evYieldPacked >> 6) & 0x3);
        pkmn.setSpatkEvYield((evYieldPacked >> 8) & 0x3);
        pkmn.setSpdefEvYield((evYieldPacked >> 10) & 0x3);
        ...
        // Gender, Hatch Cycles, Base Happiness
        pkmn.setGenderRatio(stats[Gen6Constants.bsGenderOffset] & 0xFF);
        pkmn.setHatchCounter(stats[Gen6Constants.bsHatchCyclesOffset] & 0xFF);
        pkmn.setBaseHappiness(stats[Gen6Constants.bsBaseHappinessOffset] & 0xFF);

        // Egg Groups
        pkmn.setEggGroup1(stats[Gen6Constants.bsEggGroup1Offset] & 0xFF);
        pkmn.setEggGroup2(stats[Gen6Constants.bsEggGroup2Offset] & 0xFF);

        // Color and Flip (packed in byte at 0x21)
        int colorFlags = stats[Gen6Constants.bsColorFlagsOffset] & 0xFF;
        pkmn.setColor(colorFlags & 0x3F); // bits 0-5
        pkmn.setFlip((colorFlags & 0x40) != 0); // bit 6

        // Base EXP (ushort at 0x22)
        pkmn.setExpYield(FileFunctions.read2ByteInt(stats, Gen6Constants.bsBaseEXPOffset));

        // Height and Weight (ushorts)
            ...
            if (!altFormes.containsKey(pkmn.getNumber())) {
                ...
                if (firstFormeOffset != 0) {
                    for (int i = 1; i < formeCount; i++) {
                        altFormes.put(firstFormeOffset + i - 1, new FormeInfo(pkmn.getNumber(), i)); // Assumes that
                                                                                                     // formes are in
                                                                                                     // memory in the
                                                                                                     // same order as
                                                                                                     // their numbers
                        if (Gen6Constants.actuallyCosmeticForms.contains(firstFormeOffset + i - 1)) {
                            if (pkmn.getNumber() != SpeciesIDs.pikachu && pkmn.getNumber() != SpeciesIDs.cherrim) { // No
                                                                                                                    // Pikachu/Cherrim
                    ...
                    if (pkmn.getNumber() != SpeciesIDs.arceus && pkmn.getNumber() != SpeciesIDs.genesect
                            && pkmn.getNumber() != SpeciesIDs.xerneas) {
            ...
            for (int i = 1; i <= Gen6Constants.pokemonCount + Gen6Constants.getFormeCount(romEntry.getRomType()); i++) {
                ...
                for (int evo = 0; evo < 8; evo++) {
                    ...
                    if (method >= 1 && method <= Gen6Constants.evolutionMethodCount && species >= 1) {
                        ...
                        if (et.equals(EvolutionType.LEVEL_HIGH_BEAUTY))
                            continue; // Remove Feebas "split" evolution
    ...
    private void loadMoves() {
        try {
            ...
            for (int i = 1; i <= moveCount; i++) {
                ...
                moves[i].criticalStage = critStages;
                if (critStages == 6) {
                ...
                int internalStatusType = rawStatusEffect;
                ...
                moves[i].isProtectedFromProtect = (flags & 0x008) != 0;
                moves[i].isMagicCoatAffected = (flags & 0x010) != 0;
                moves[i].isSnatchAffected = (flags & 0x020) != 0;
                moves[i].isMirrorMoveAffected = (flags & 0x040) != 0;
                moves[i].isPunchMove = (flags & 0x080) != 0;
                moves[i].isSoundMove = (flags & 0x100) != 0;
                moves[i].groundedByGravity = (flags & 0x200) != 0;
                moves[i].defrostsUser = (flags & 0x400) != 0;
                moves[i].hitsNonAdjacent = (flags & 0x800) != 0;
                ...
                moves[i].isTrapMove = moves[i].isTrapMove
                        || moves[i].minTrapTurns > 0
                        || moves[i].maxTrapTurns > 0
                        || moves[i].effectIndex == Gen6Constants.noDamageTargetTrappingEffect
                        || moves[i].effectIndex == Gen6Constants.noDamageFieldTrappingEffect
                        || moves[i].effectIndex == Gen6Constants.damageAdjacentFoesTrappingEffect
                        || rawStatusEffect == 8;
    ...
    protected String getGameVersion() {
        ...
        // This shouldn't be seen by users, but is correct assuming we accidentally show
        // it to them.
        return "Unpatched";
    }
    ...
    public void savePokemonStats() {
        ...
        byte[] duplicateData = pokeGarc.files
                .get(Gen6Constants.pokemonCount + Gen6Constants.getFormeCount(romEntry.getRomType()) + 1).get(0);
        for (int i = 1; i <= Gen6Constants.pokemonCount + Gen6Constants.getFormeCount(romEntry.getRomType()); i++) {
    ...
    private void saveBasicPokeStats(Species pkmn, byte[] stats) {
        ...
        // EV Yields - pack into ushort at offset 0x0A
        int evYieldPacked = (pkmn.getHpEvYield() & 0x3) |
                ((pkmn.getAttackEvYield() & 0x3) << 2) |
                ((pkmn.getDefenseEvYield() & 0x3) << 4) |
                ((pkmn.getSpeedEvYield() & 0x3) << 6) |
                ((pkmn.getSpatkEvYield() & 0x3) << 8) |
                ((pkmn.getSpdefEvYield() & 0x3) << 10);
        FileFunctions.write2ByteInt(stats, Gen6Constants.bsEVYieldOffset, evYieldPacked);

        // Held items
        if (pkmn.getGuaranteedHeldItem() != null) {
            FileFunctions.write2ByteInt(stats, Gen6Constants.bsCommonHeldItemOffset,
                    pkmn.getGuaranteedHeldItem().getId());
        ...
        if (pkmn.getFullName().equals("Meowstic")) {
            ...
            stats[Gen6Constants.bsGenderOffset] = (byte) pkmn.getGenderRatio();
        }
        stats[Gen6Constants.bsHatchCyclesOffset] = (byte) pkmn.getHatchCounter();
        stats[Gen6Constants.bsBaseHappinessOffset] = (byte) pkmn.getBaseHappiness();

        stats[Gen6Constants.bsGrowthCurveOffset] = pkmn.getGrowthCurve().toByte();

        // Egg Groups
        stats[Gen6Constants.bsEggGroup1Offset] = (byte) pkmn.getEggGroup1();
        stats[Gen6Constants.bsEggGroup2Offset] = (byte) pkmn.getEggGroup2();

        stats[Gen6Constants.bsAbility1Offset] = (byte) pkmn.getAbility1();
        stats[Gen6Constants.bsAbility2Offset] = pkmn.getAbility2() != 0 ? (byte) pkmn.getAbility2()
                : (byte) pkmn.getAbility1();
        stats[Gen6Constants.bsAbility3Offset] = (byte) pkmn.getAbility3();

        // Color and Flip - pack into byte at offset 0x21
        int colorFlags = (pkmn.getColor() & 0x3F) | (pkmn.isFlip() ? 0x40 : 0);
        stats[Gen6Constants.bsColorFlagsOffset] = (byte) colorFlags;

        // Base EXP (ushort at 0x22)
        FileFunctions.write2ByteInt(stats, Gen6Constants.bsBaseEXPOffset, pkmn.getExpYield());
    ...
    private void writeShedinjaEvolution() throws IOException {
        ...
        // new primary evolution; that way, evolving Nincada will still produce an
        // "extra"
        ...
        // We can't just blindly replace it, though, because certain constants (for
        // example,
        // 0x125) cannot be moved without using the movw instruction. This works fine in
        // Citra, but crashes on real hardware. Instead, we have to annoyingly shift up
        // a
        ...
        if (offset > 0) {
            ...
            // Shift up everything below the last nop to make some room at the bottom of the
            // function.
            for (int i = 80; i < 188; i++) {
                code[offset + i] = code[offset + i + 4];
            }

            // For every bl that we shifted up, patch them so they're now pointing to the
            // same place they
            // were before (without this, they will be pointing to 0x4 before where they're
            // supposed to).
    ...
    public void saveMoves() {
        ...
        for (int i = 1; i <= moveCount; i++) {
            ...
            data[0] = Gen6Constants.typeToByte(move.type);
            data[1] = (byte) clamp(move.categoryQuality, 0, 255);
            data[2] = Gen6Constants.moveCategoryToByte(move.category);
            ...
            int flags = FileFunctions.readFullInt(data, 32) & 0xFFFF0000;
    ...
    private void patchFormeReversion() throws IOException {
        ...
        // In ORAS, the game also has hardcoded checks to revert Primal Groudon and
        // Primal Kyogre
        ...
        if (romEntry.getRomType() == Gen6Constants.Type_ORAS) {
            ...
            if (offset > 0) {
                ...
                // The game checks for Primal Kyogre and Primal Groudon by pc-relative loading
                // 0x17E,
                // which is Kyogre's species ID. The call to
                // pml::pokepara::CoreParam::ChangeFormNo
                // is used by other species which we probably don't want to break, so instead of
                // stubbing the call to the function, just break the hardcoded species ID check
                // by
    ...
    public Species getAltFormeOfSpecies(Species base, int forme) {
        int pokeNum = absolutePokeNumByBaseForme.getOrDefault(base.getNumber(), dummyAbsolutePokeNums)
    ...
    public List<Species> getStarters() {
        ...
        try {
            ...
            List<Integer> starterIndices = Arrays.stream(romEntry.getArrayValue("StarterIndices")).boxed()
    ...
    public boolean setStarters(List<Species> newStarters) {
        ...
        // However, since randomness was factored out of the RomHandlers, that
        // functionality was simply removed.
        ...
        try {
            ...
            int size = Gen6Constants.getGiftPokemonSize(romEntry.getRomType());
            int offset = romEntry.getIntValue("GiftPokemonOffset");
            int displayOffset = readWord(displayCRO, romEntry.getIntValue("StarterOffsetOffset"))
                    + romEntry.getIntValue("StarterExtraOffset");
    ...
    private List<EncounterArea> getEncountersXY() throws IOException {
        ...
        for (int i = 0; i < encounterGarc.files.size() - 1; i++) {
            ...
            // First, 12 grass encounters, 12 rough terrain encounters, and 12 encounters
            // each for yellow/purple/red flowers
        ...
        for (int i = 0; i < Gen6Constants.rustlingBushEncounterCount; i++) {
            ...
            int encounterNumber = (i - startingOffsetOfCurrentName) + 1;
            rustlingBushArea.setIdentifiers(currentName + " #" + encounterNumber, currentMapIndex,
    ...
    private void updatePokedexAreaDataORAS(GARCArchive encounterGarc) throws IOException {
        byte[] pokedexAreaData = new byte[(Gen6Constants.pokemonCount + 1)
                * Gen6Constants.perPokemonAreaDataLengthORAS];
        int currentMapNum = 0;
        for (int i = 0; i < encounterGarc.files.size() - 2; i++) {
            byte[] b = encounterGarc.files.get(i).get(0);
            int offset = FileFunctions.readFullInt(b, 0x10) + 0xE;
    ...
    private void updatePokedexAreaDataFromEncounterArea(EncounterArea area, byte[] pokedexAreaData, int areaIndex,
            int encounterType) {
        for (Encounter enc : area) {
            Species pk = enc.getSpecies();
            int perPokemonAreaDataLength = romEntry.getRomType() == Gen6Constants.Type_XY
                    ? Gen6Constants.perPokemonAreaDataLengthXY
                    : Gen6Constants.perPokemonAreaDataLengthORAS;
                    ...
                    switch (tr.currBattleStyle.getStyle()) {
                        ...
                        case DOUBLE_BATTLE:
                            if (trainer[offset + 2] != 1) {
                                trainer[offset + 2] = 1;
                                trainer[offset + 12] |= (byte) 0x80; // Flag that needs to be set for trainers not to
                                                                     // attack their own pokes
                            }
                            break;
                        case TRIPLE_BATTLE:
                            if (trainer[offset + 2] != 2) {
                                trainer[offset + 2] = 2;
                                trainer[offset + 12] |= 0x80; // Flag that needs to be set for trainers not to attack
                                                              // their own pokes
                            }
                            break;
                    ...
                    if (tr.pokemonHaveCustomMoves()) {
                        if (tp.isResetMoves()) {
                            int[] pokeMoves = RomFunctions.getMovesAtLevel(
                                    getAltFormeOfSpecies(tp.getSpecies(), tp.getForme()).getNumber(), movesets,
                                    tp.getLevel());
                            for (int m = 0; m < 4; m++) {
    ...
    public void setEggMoves(Map<Integer, List<Integer>> eggMoves) {
        try {
            ...
            for (int i = 1; i <= Gen6Constants.pokemonCount; i++) {
                ...
                // Reallocate per species so legacy zero-length buffers do not cap the move
                // list.
                byte[] updated = new byte[2 + (moveCount * 2)];

                writeWord(updated, 0, moveCount);
                for (int j = 0; j < moveCount; j++) {
                    writeWord(updated, 2 + (j * 2), moves.get(j));
                }
    ...
    public List<StaticEncounter> getStaticPokemon() {
        ...
        try {
            ...
            List<Integer> skipStarters = Arrays.stream(romEntry.getArrayValue("StarterIndices")).boxed()
                    .collect(Collectors.toList());
            ...
            if (romEntry.getRomType() == Gen6Constants.Type_XY) {
                ...
                if (tableBaseOffset > 0) {
                    tableBaseOffset += Gen6Constants.xyTrashEncountersTablePrefix.length() / 2; // because it was a
                                                                                                // prefix
                    statics.addAll(readTrashCanEncounterSet(tableBaseOffset, Gen6Constants.pokemonVillageGarbadorOffset,
                            Gen6Constants.pokemonVillageGarbadorCount, true));
                    statics.addAll(readTrashCanEncounterSet(tableBaseOffset, Gen6Constants.pokemonVillageBanetteOffset,
                            Gen6Constants.pokemonVillageBanetteCount, true));
                    statics.addAll(readTrashCanEncounterSet(tableBaseOffset, Gen6Constants.lostHotelGarbadorOffset,
                            Gen6Constants.lostHotelGarbadorCount, true));
                    statics.addAll(readTrashCanEncounterSet(tableBaseOffset, Gen6Constants.lostHotelTrubbishOffset,
                            Gen6Constants.lostHotelTrubbishCount, true));
                    statics.addAll(readTrashCanEncounterSet(tableBaseOffset, Gen6Constants.lostHotelRotomOffset,
                            Gen6Constants.lostHotelRotomCount, false));
                }
            }
        } catch (IOException e) {
            throw new RomIOException(e);
    ...
    private void fixBoxLegendariesXY(int boxLegendarySpecies) throws IOException {
        // We need to edit the script file or otherwise the text will still say
        // "Xerneas" or "Yveltal"
        GARCArchive encounterGarc = readGARC(romEntry.getFile("WildPokemon"), false);
        ...
        System.arraycopy(modifiedScript, 0, boxLegendaryRoomData, Gen6Constants.boxLegendaryLocalScriptOffsetXY,
        ...
        if (functionOffset > 0) {
            ...
            // At multiple points in the function, the game calls
            // pml::pokepara::CoreParam::GetMonNo
            // and compares the result to r8; every single one of these comparisons is
            // followed by a
            // nop. However, the way in which the species ID is loaded into r8 differs
            // depending on
            // the game. We'd prefer to write the same assembly for both games, and there's
            // a trick
            // we can abuse to do so. Since the species ID is never used outside of this
            // comparison,
            // we can feel free to mutate it however we please. The below code allows us to
            // write any
    ...
    private void setRoamersXY(List<StaticEncounter> staticPokemon) throws IOException {
        ...
        if (roamerSpeciesOffset > 0 && freeSpaceOffset > 0) {
            // In order to make this code work with all versions of XY, we had to find the
            // *end* of our free space.
            ...
            // nop
            // b LAB_003b7d1c
            // We want to move both branches to the top so that we have 12 bytes of space to
            // work with.
            // Start by moving "bl FUN_0041b710" up one instruction, making sure to adjust
            ...
            // Now move "b LAB_003b7d1c" up three instructions, again adjusting the branch
            // accordingly.
            ...
            // To load the species ID, the game currently does "moveq r4, #0x90" for
            // Articuno and similar
            // things for Zapdos and Moltres. Instead, just pc-relative load what we wrote
            // before. The fact
            // that we change the conditional moveq to the unconditional pc-relative load
            // only matters for
            // the case where the player's starter index is *not* 0, 1, or 2, but that can't
            // happen outside
            // of save editing.
            for (int i = 0; i < roamers.length; i++) {
        ...
        try {
            ...
            System.arraycopy(modifiedScript, 0, seaSpiritsDenAreaFile, Gen6Constants.seaSpiritsDenLocalScriptOffsetXY,
                    modifiedScript.length);
    ...
    private void fixRayquazaORAS(int rayquazaEncounterSpecies) throws IOException {
        ...
        // We also need to edit DllField.cro so that the hardcoded checks for Rayquaza's
        // species
    ...
    private void patchForNationalDex() {
        ...
        if (romEntry.getRomType() == Gen6Constants.Type_XY) {
            ...
            if (offset > 0) {
                ...
                // the player owns. It can be called with 0 (Central), 1 (Coastal), 2
                // (Mountain),
                ...
                // The below code nops out that comparison and makes the move and branch
                // instructions
                // non-conditional; no matter what's on the save file, the player will have all
                // dexes.
            ...
            // DllSangoZukan.cro will refuse to let you open either the Hoenn or National
            // Pokedex if you have
            // caught 0 Pokemon in the Hoenn Pokedex; it is unknown *how* it does this,
            // though. Instead, let's
            // just hack up the function that determines how many Pokemon in the Hoenn
            // Pokedex you've caught so
            ...
            if (offset > 0) {
                ...
                // At the start of the function, there's a check that the Zukan block on the
                // save data is valid;
                // this is obviously generated by either a macro or inlined function, since
                // literally every function
                // relating to the Pokedex has this too. First, it checks if the checksum is
                // correct then does a beq
                // to branch to the main body of the function; let's replace this with an
                // unconditional branch.
                code[offset + 31] = (byte) 0xEA;

                // Now, in the space where the function would normally handle the call to the
                // assert function
                // to crash the game if the checksum is invalid, we can write the following
                // code:
                // mov r0, r7
                // cmp r0, #0x0
                // moveq r0, #0x1
                ...
                // At the end of the function, the game normally does "mov r0, r7" and then
                // returns, where r7
                // contains the number of Pokemon caught in the Hoenn Pokedex. Instead, branch
                // to the code we
                // wrote above.
    ...
    public void enableGuaranteedPokemonCatching() {
        try {
            ...
            if (offset > 0) {
                // The game checks to see if your odds are greater then or equal to 255 using
                // the following
                // code. Note that they compare to 0xFF000 instead of 0xFF; it looks like all
                // catching code
                // probabilities are shifted like this?
                // cmp r6, #0xFF000
                // blt oddsLessThanOrEqualTo254
                // The below code just nops the branch out so it always acts like our odds are
                // 255, and
        ...
        if (offset > 0) {
            ...
            for (int i = 0; i < Gen6Constants.tmBlockThreeCount; i++) {
                itemDescriptions.set(i + Gen6Constants.tmBlockThreeOffset,
                        moveDescriptions.get(
                                moveIndexes.get(i + Gen6Constants.tmBlockOneCount + Gen6Constants.tmBlockTwoCount)));
            }
            ...
            if (offsPals > 0) {
                ...
                for (int i = 0; i < (Gen6Constants.tmBlockThreeCount); i++) {
                    ...
                    Move m = this.moves[moveIndexes
                            .get(i + Gen6Constants.tmBlockOneCount + Gen6Constants.tmBlockTwoCount)];
                    int pal = this.typeTMPaletteNumber(m.type, false);
                    writeWord(code, offsPals + itmNum * 4, pal);
                }
            }
    ...
    private static final byte[] ORAS_TUTOR_LOCATOR_VANILLA = asciiBytes("\0FormatType\0\0EdgeID\0\u00FF");
    private static final byte[] ORAS_TUTOR_LOCATOR_PATCHED = asciiBytes(
            "\0FormatType\0\0EdgeID\0\0c:\\revision1_sango\\sango_project\\prog\\src/system/motion/Motion.cpp\0\0");
    ...
    private int getMoveTutorMovesOffset() {
        if (moveTutorMovesOffset != 0) {
            return moveTutorMovesOffset;
        }

        int offset = findMoveTutorMovesOffsetFromMarker();
        if (offset <= 0) {
    ...
    private int findMoveTutorMovesOffsetFromMarker() {
        if (!hasMoveTutors() || code == null) {
            return -1;
        }

        int offset = indexOf(code, ORAS_TUTOR_LOCATOR_VANILLA, 0);
        if (offset >= 0) {
            return offset + ORAS_TUTOR_LOCATOR_VANILLA.length;
        }

        offset = indexOf(code, ORAS_TUTOR_LOCATOR_PATCHED, 0);
        if (offset >= 0) {
            return offset + ORAS_TUTOR_LOCATOR_PATCHED.length;
        }

        return -1;
    }

    private int indexOf(byte[] haystack, byte[] needle, int startIndex) {
    ...
    public SpeciesSet getBannedForStaticPokemon() {
        return new SpeciesSet(Gen6Constants.actuallyCosmeticForms
                .stream()
                .filter(index -> index < Gen6Constants.pokemonCount
                        + Gen6Constants.getFormeCount(romEntry.getRomType()))
            ...
            if (pkmn != null) {
                for (Evolution evo : pkmn.getEvolutionsFrom()) {
                    ...
                    if (evo.getType() == EvolutionType.TRADE_SPECIAL) {
                        ...
                        evo.setExtraInfo((evo.getFrom().getNumber() == SpeciesIDs.karrablast ? SpeciesIDs.shelmet
                                : SpeciesIDs.karrablast));
                    }
    ...
    public boolean setIntroPokemon(Species pk) {
        ...
        try {
            ...
            // Then change the parameters that are loaded for a function call, and also
            // change the function call
            // itself to a function that uses the "cry value" instead of Pokemon ID + forme
            ...
            writeLong(introCRO, croInitialCryOffset3 + 0x10, readLong(introCRO, croInitialCryOffset3 + 0x14)); // Move
                                                                                                               // these
                                                                                                               // two
                                                                                                               // instructions
                                                                                                               // up
    ...
    public List<Integer> getFieldItemIds() {
        ...
        try {
            ...
            // TODO: make all field mega stones supported, XY and ORAS, but shuffle them
            ...
            // In ORAS, it's possible to encounter the sparkling Mega Stone items on the
            // field
    ...
    private List<Integer> getFieldMegaStonesORAS(GARCArchive scriptGarc) throws IOException {
        ...
        for (int i = 0; i < Gen6Constants.megastoneTableLengthORAS; i++) {
            int offset = Gen6Constants.megastoneTableStartingOffsetORAS
            ...
            if (romEntry.getRomType() == Gen6Constants.Type_ORAS) {
                ...
                for (int i = 0; i < Gen6Constants.megastoneTableLengthORAS; i++) {
                    ...
                    if (megaStoneMap.containsKey(oldItem)) {
                        // There are some duplicate entries for certain Mega Stones, and we're not quite
                        // sure why.
    ...
    public void setInGameTrades(List<InGameTrade> trades) {
        ...
        if (offset > 0) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                FileFunctions.write2ByteInt(code, offset + 0x10,
                        trade.getHeldItem() == null ? 0 : trade.getHeldItem().getId());
                ...
                // In XY, there are some trades that use hardcoded strings. Go and forcibly
                // update
                // the story text so that the trainer says what they want to trade.
                if (romEntry.getRomType() == Gen6Constants.Type_XY
                        && Gen6Constants.xyHardcodedTradeOffsets.contains(i)) {
                    int hardcodedTradeIndex = Gen6Constants.xyHardcodedTradeOffsets.indexOf(i);
                    updateHardcodedTradeText(oldTrades.get(i), trade,
                            Gen6Constants.xyHardcodedTradeTexts.get(hardcodedTradeIndex));
                }
            }
    ...
    // NOTE: This method is kind of stupid, in that it doesn't try to reflow the
    // text to better fit; it just
    // blindly replaces the Pokemon's name. However, it seems to work well enough
    // for what we need.
    private void updateHardcodedTradeText(InGameTrade oldTrade, InGameTrade newTrade, int hardcodedTradeTextFile) {
        ...
        for (int i = 0; i < hardcodedTradeStrings.size(); i++) {
            ...
            if (oldRequestedName != null && newRequestedName != null
                    && hardcodedTradeString.contains(oldRequestedName)) {
    ...
    public List<Item> getSensibleHeldItemsFor(TrainerPokemon tp, boolean consumableOnly, List<Move> moves,
        ...
        Map<Type, Effectiveness> byType = getTypeTable().against(tp.getSpecies().getPrimaryType(false),
        ...
        } else if (byType.get(Type.GROUND) == Effectiveness.DOUBLE
                || byType.get(Type.GROUND) == Effectiveness.QUADRUPLE) {
        ...
        if (!consumableOnly) {
            ...
            if (tp.getSpecies().getPrimaryType(false) == Type.POISON
                    || tp.getSpecies().getSecondaryType(false) == Type.POISON) {
            ...
            if (!tp.getSpecies().getEvolutionsFrom().isEmpty() && tp.getLevel() >= 20) {
                // eviolite can be too good for early game, so we gate it behind a minimum
                // level.
