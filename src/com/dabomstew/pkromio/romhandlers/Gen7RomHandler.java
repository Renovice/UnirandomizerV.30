...
public class Gen7RomHandler extends Abstract3DSRomHandler {
    ...
    int evYieldPacked = FileFunctions.read2ByteInt(stats, Gen7Constants.bsEVYieldOffset);
    ...
    pkmn.setGrowthCurve(ExpCurve.fromByte(stats[Gen7Constants.bsGrowthCurveOffset]));
        ...
        pkmn.setGenderRatio(stats[Gen7Constants.bsGenderOffset] & 0xFF);
        pkmn.setHatchCounter(stats[Gen7Constants.bsHatchCyclesOffset] & 0xFF);
        pkmn.setBaseHappiness(stats[Gen7Constants.bsBaseHappinessOffset] & 0xFF);

        pkmn.setEggGroup1(stats[Gen7Constants.bsEggGroup1Offset] & 0xFF);
        pkmn.setEggGroup2(stats[Gen7Constants.bsEggGroup2Offset] & 0xFF);

        int colorFlags = stats[Gen7Constants.bsColorFlagsOffset] & 0xFF;
        pkmn.setColor(colorFlags & 0x3F);
        pkmn.setFlip((colorFlags & 0x40) != 0);

        pkmn.setExpYield(FileFunctions.read2ByteInt(stats, Gen7Constants.bsBaseEXPOffset));
    ...
    // Pack EV yield bits into the personal data field
    ...
    FileFunctions.write2ByteInt(stats, Gen7Constants.bsEVYieldOffset, evYieldPacked);
        ...
        stats[Gen7Constants.bsGenderOffset] = (byte) pkmn.getGenderRatio();
        stats[Gen7Constants.bsHatchCyclesOffset] = (byte) pkmn.getHatchCounter();
        stats[Gen7Constants.bsBaseHappinessOffset] = (byte) pkmn.getBaseHappiness();

        stats[Gen7Constants.bsEggGroup1Offset] = (byte) pkmn.getEggGroup1();
        stats[Gen7Constants.bsEggGroup2Offset] = (byte) pkmn.getEggGroup2();

        int colorFlags = (pkmn.getColor() & 0x3F) | (pkmn.isFlip() ? 0x40 : 0);
        stats[Gen7Constants.bsColorFlagsOffset] = (byte) colorFlags;

        FileFunctions.write2ByteInt(stats, Gen7Constants.bsBaseEXPOffset, pkmn.getExpYield());
